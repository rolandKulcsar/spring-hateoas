...
import org.springframework.hateoas.affordance.springmvc.DefaultDocumentationProvider;
import org.springframework.hateoas.affordance.springmvc.DocumentationProvider;
 ...
 * &quot;tunnel&quot; other methods when this converter is used with the browser. Spring's
 * {@link org.springframework.web.filter.HiddenHttpMethodFilter} allows to do that with relative ease.
 *
...
public class XhtmlResourceMessageConverter extends AbstractHttpMessageConverter<Object> {
	...
	/**
	 * Sets method param name for HTML PUT/DELETE/PATCH workaround. Defaults to {@literal _method}.
	 *
	 ...
	 *
	 * @param stylesheets urls of css stylesheets to include, e.g.
	 *          &quot;https://maxcdn.bootstrapcdn.com/bootstrap/3.3.4/css/bootstrap.min.css&quot;
	 */
	public void setStylesheets(List<String> stylesheets) {

		Assert.notNull(stylesheets, "stylesheets must not be null!");
	 ...
	 */
	private static InputStream getBodyFromServletRequestParameters(HttpServletRequest request, String charset)
			throws IOException {
	...
	private static Object readRequestBody(Class<?> clazz, InputStream inputStream, Charset charset) throws IOException {
	...
	public static Object recursivelyCreateObject(Class<?> clazz, MultiValueMap<String, String> formValues,
		...
		if (Map.class.isAssignableFrom(clazz)) {
			...
			try {
				Constructor<?> constructor = PropertyUtils.findDefaultConstructor(clazz);
				if (constructor == null) {
					constructor = PropertyUtils.findConstructorByAnnotation(clazz, JsonCreator.class);
				}
						...
						for (Annotation annotation : annotationsOnParameter) {
							if (JsonProperty.class == annotation.annotationType()) {
								...
								if (DataTypeUtils.isSingleValueType(parameterType)) {
									if (formValue != null) {
										if (formValue.size() == 1) {
											args[paramIndex++] = DataTypeUtils.asType(parameterType, formValue.get(0));
										} else {
				...
				for (PropertyDescriptor propertyDescriptor : propertyDescriptors) {
					...
					if (writeMethod != null && strings != null && strings.size() == 1) {
						writeMethod.invoke(ret, DataTypeUtils.asType(propertyDescriptor.getPropertyType(), strings.get(0))); // TODO
																																																						// lists,
																																																						// consume
																																																						// values
																																																						// from ctor
	...
	private void writeObject(XhtmlWriter writer, Object object)
		...
		if (!DataTypeUtils.isSingleValueType(object.getClass())) {
			writer.beginDl();
		}
	...
	private static boolean contentIsEmpty(Object content) {
	...
	private static void writeDtWithDoc(XhtmlWriter writer, String name, String documentationUrl) throws IOException {
		...
		if (documentationUrl == null) {
			...
			writer.beginDt();
			writer.beginAnchor(XhtmlWriter.OptionalAttributes.attr("href", documentationUrl).and("title", documentationUrl));
			writer.write(name);
			writer.endAnchor();
			writer.endDt();
		}
	...
	private static void writeDdForScalarValue(XhtmlWriter writer, Object value) throws IOException {
		writer.beginDd();
