...
import static org.springframework.hateoas.xhtml.XhtmlWriter.OptionalAttributes.*;
...
public class XhtmlWriter extends Writer {
	...
	public static class OptionalAttributes {
		...
		public static OptionalAttributes attr(final String name, final String value) {
			Assert.isTrue(name != null && value != null || value == null, "name/value must be valid!");
					...
					if (affordance.isTemplated()) {
						...
						if (!affordance.isBaseUriTemplated()) {
							for (ActionDescriptor actionDescriptor : actionDescriptors) {
								...
								if (HttpMethod.GET.equals(actionDescriptor.getHttpMethod())) {
						...
						for (ActionDescriptor actionDescriptor : actionDescriptors) {
							...
							if (HttpMethod.GET.equals(actionDescriptor.getHttpMethod())
									&& actionDescriptor.getRequestParamNames().isEmpty()) {
								beginDiv();
	...
	private void appendForm(final Affordance affordance, final ActionDescriptor actionDescriptor) throws IOException {
		...
		HttpMethod httpMethod = actionDescriptor.getHttpMethod();
	...
	private void writeHiddenHttpMethodField(HttpMethod httpMethod) throws IOException {
	...
	private String getHtmlConformingHttpMethod(HttpMethod requestMethod) {
