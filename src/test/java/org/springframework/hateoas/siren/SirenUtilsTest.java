...
public class SirenUtilsTest {
	...
	public class ProfileResource {
		...
		@JsonUnwrapped private final Resources<EmbeddedWrapper> embeddeds;
	...
	public void testEmbeddedLink() {
		...
		String json = objectMapper.valueToTree(entity).toString();
		assertThat(json, hasJsonPath("$.entities[0].rel", contains("address")));
		assertThat(json, hasJsonPath("$.entities[0].href", equalTo("http://api.example.com/customers/123/address")));
	}
	...
	public void testAttributeWithMap() {
		...
		assertThat(json, hasJsonPath("$.properties.address.city.postalCode", equalTo("74199")));
	}
	...
	public void testAttributeWithListOfEnums() {
		...
		class Customer {
			...
			List<Daytime> favoriteNumbers = Arrays.asList(Daytime.AFTERNOON, Daytime.NIGHT);
		...
		assertThat(json,
				hasJsonPath("$.properties.favoriteNumbers", contains(Daytime.AFTERNOON.name(), Daytime.NIGHT.name())));
		assertThat(json, hasJsonPath("$.properties.name", equalTo("Peter Joseph")));
	}
