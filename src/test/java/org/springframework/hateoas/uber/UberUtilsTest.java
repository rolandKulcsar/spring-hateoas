...
public class UberUtilsTest {
	...
	public void linkGetToUberNode() throws Exception {
		UberNode linkNode = UberUtils.toUberLink("/foo", new SpringActionDescriptor("get", HttpMethod.GET), Link.REL_SELF);
	...
	public void linkPostToUberNode() throws Exception {
		...
		UberNode linkNode = UberUtils.toUberLink("/foo{?foo,bar}", new SpringActionDescriptor("post", HttpMethod.POST),
				Link.REL_SELF);
		assertEquals(Arrays.asList(Link.REL_SELF), linkNode.getRel());
		assertEquals("/foo", linkNode.getUrl());
