...
public class XhtmlResponseTest {
	...
	public void testRequestWithStatusFound() throws Exception {
		...
		for (Item item : dm.items) {
			...
			// REMOTE
			assertInputProperty(htmlStr, "select", "searchedSubItem",
			...
			XMLAssert.assertXpathExists(
					"//select[@name='searchedSubItem'][@data-remote='http://localhost/test/subitem/filter{?filter}']", htmlStr);
			// REMOTE
			...
			XMLAssert.assertXpathExists(
					"//select[@name='another'][@data-remote='http://localhost/test/subitem/anotherFilter/{filter}/']", htmlStr);
			...
			assertInputProperty(htmlStr, "select", "subEntity.multiple",
					DummyController.SUBENTITY_MULTIPLE_READONLY.contains(index),
					DummyController.SUBENTITY_MULTIPLE_REQUIRED.contains(index), item.getSubEntity().getMultiple().toArray(),
					false);
			assertInputProperty(htmlStr, "number", "listSubEntity[0].lkey",
					DummyController.LIST_SUBENTITY_KEY_READONLY.contains(index),
					DummyController.LIST_SUBENTITY_KEY_REQUIRED.contains(index),
					Integer.toString(item.getListSubEntity().get(0).getLkey()), false);
	...
	private void assertInputProperty(final String htmlStr, final String htmlType, final String name,
			final boolean readOnly, final boolean required, final Object valueObj, final boolean isHidden)
			...
			if (isHidden) {
				...
				if (!"select".equals(htmlType)) {
					...
					if (value != null && !"".equals(value)) {
						XMLAssert.assertXpathExists("//select[@name='" + name + "']/option[@selected][@value='" + value + "']",
								htmlStr);
						if (readOnly) {
							XMLAssert.assertXpathExists("//input[@type='hidden'][@name='" + name + "'][@value='" + value + "']",
									htmlStr);
						}
					}
				}
